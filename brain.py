...
class Brain(object):
    ...
    def _build_model(self):
        ...
        if self.dueling:
            ...
            y1 = Dense(self.num_nodes, activation='relu', trainable=False)(x)
            y2 = Dense(self.num_nodes, activation='relu', trainable=False)(y1)
    ...
    def grad_model(self, x):

        # loss = self.model.evaluate(x, y, batch_size=len(x), sample_weight=sample_weight, verbose=verbose)
        # print(loss)
        # print(self.model.loss(self.model_.output, self.model.output))
        grad = K.gradients(self.model.loss(self.model_.output, self.model.output), [self.model.input, self.model_.input])[0]
        # sess = tf.InteractiveSession()
        # sess.run(tf.initialize_all_variables())
        with tf.Session() as sess:
            tf.global_variables_initializer().run()
            evaluated_gradients = sess.run(grad, feed_dict={self.model.input: x, self.model_.input: x})
            sign = sess.run(tf.math.sign(evaluated_gradients[0]))
            # eps = 0.55 * sign  #agent_landmarks
            # eps = 0.5 * sign  # predators_prey
        # sess.close()
        return sign
